...
| L9 | `canary_expectations.json`'da `pipeline` anahtarı hiç okunmuyor (ölü) | **RESOLVED** | `test_long_query_rank_ratchet` artık `block["pipeline"] == s.retrieval_pipeline` assert ediyor |
