...
| G0.2 | Sorgu A + B kalıcı regression setinde | **PASS** | `tests/retrieval/test_semantic_retrieval_eval.py` (6 slow test); `tests/retrieval/retrieval_regression_expectations.json` cırcır: `hybrid.long_query_gold_rank_max=2`, `exhaustive=664` |
...
| **G1.4** | Sorgu A dayanak sayfası final top-5 (zorunlu regression) | **FİİLEN KARŞILANDI, RAPORA YAZILMADI** | `tests/retrieval/retrieval_regression_expectations.json` → `hybrid.long_query_gold_rank_max = 2` (ölçüm 2026-08-30, üretim indeksi + hibrit). Yani rank 2 ≤ 5. P0 devrindeki D3 (1221 → 664 → **2**) böylece kapandı. |
