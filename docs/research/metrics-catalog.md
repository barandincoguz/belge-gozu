...
| `verifier_ms` | REAL, NULL olabilir | ms | `answer/base.py` `with stage("verifier")` | kanıt doğrulayıcı çağrılarının toplam süresi; yalnız gate2 çalıştığında | 2 |
...
| `tokens_in` | INTEGER | token | `app/main.py` (`detail.llm_usage`) | bilinen yanıtlayıcı + verifier token toplamı; metadata yoksa NULL | 1 |
| `tokens_out` | INTEGER | token | `app/main.py` (`detail.llm_usage`) | bilinen toplam üretim hacmi; metadata yoksa NULL | 1 |
| `tokens_per_s` | REAL | token/sn | `app/main.py` (yalnız answerer token'ları / `answer_ms`) | akışsız yanıtlayıcı üretim hızı; verifier süresi/tokenı bu oranı kirletmez | 1 |
...
| `bg_stage_duration_seconds` | Histogram | `stage` ∈ {query_encode, exhaustive_maxsim, text_bm25, route_fuse, stage1_hamming, stage2_maxsim, answerer, verifier} | saniye | `prom.py: self.stage`, bucket'lar `STAGE_BUCKETS` | darboğaz ayrıştırması: "gecikme nerede birikiyor?" | 1 |
...
| `bg_llm_tokens_total` | Counter | `direction` ∈ {input, output} | token | `prom.py: self.tokens` (`RequestEvent.tokens_*`) | bilinen yanıtlayıcı + verifier toplam hacmi; metadata yoksa bilinmeyen kullanım sayılmaz | 1 |
| `bg_llm_tokens_by_purpose_total` | Counter | `purpose` ∈ {answerer, verifier}, `direction` ∈ {input, output} | token | `prom.py: self.tokens_by_purpose` (`detail.llm_usage`) | yanıtlayıcı ve kanıt doğrulayıcı kullanımını ayrı izleme; toplam seriyle birlikte okunur | 2 |
...
| `bg_telemetry_write_failures_total` | Counter | — | olay yazma hatası | `prom.py: self.telemetry_write_failures` (`app/main.py` kayıt yolları) | SQLite yazımı sessizce delindiğinde operasyonel alarm sinyali | 2 |
