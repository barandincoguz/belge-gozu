...
ÇEVRİMİÇİ (HF Space, ücretsiz CPU, Docker)
  ...
  sorgu → sorgu kodlayıcı (küçük retriever, CPU)
        → Aşama 1: havuzlanmış sayfa vektörüyle Hamming eleme → top-200
        → Aşama 2: binary MaxSim (late interaction) yeniden sıralama → top-k sayfa
        → Answerer (tak-çıkar) → atıflı yanıt (sayfa küçük resmi + belge adı + resmî link)
```
...
- **Aşama 2 — kesin sıralama:** Adayların tam token matrisleriyle binary MaxSim:
  ...
  maksimumların toplamı. → top-k (varsayılan 5).
...
- **Ablasyonlar (benchmark'ta sayıya bağlanır):** binarization kaybı (float16 vs binary),
  aday sayısı (50/200/500), retriever boyutu (3B vs 500M sınıfı).
...
- Operasyonel: sorgu gecikmesi (p50/p95), yanıtlayıcı başına maliyet/1000 soru.
...
- **UI:** Tek sayfa, sade-özenli (Gradio değil): arama kutusu, sayfa küçük resimleri
  (skorlarıyla), atıf kartlı yanıt, aktif answerer rozeti. Türkçe/İngilizce dil anahtarı.
...
**Hafta 2 → asıl et:** tam korpus (~20-30 bin sayfa) + tarihî dilim; benchmark taslakları
→ kullanıcı doğrulaması; OCR baseline; eval koşuları + tablolar; LocalVLM yolu.

**Hafta 3 → tampon + cila:** ablasyonlar; README/analiz; benchmark yayını; gecikme
iyileştirmeleri; opsiyonel ZeroGPU; duyuru taslağı (LinkedIn/blog). Hafta 3 tampondur:
hafta 2 sarkarsa buradan yer.
...
| Benchmark kalitesi zayıf kalır | Kullanıcı doğrulaması zorunlu adım; dilim başına en az 25 doğrulanmış soru olmadan sonuç yayınlanmaz |
...
1. Canlı demo: HF Space'te çalışan, atıflı yanıt veren, herkese açık link.
2. `belge-gozu-bench` HF'te yayında; ≥75 kullanıcı-doğrulamalı soru, 3 dilim.
3. README'de görsel-RAG vs OCR-baseline karşılaştırma tablosu (kayıplar dahil, dürüst).
4. CI yeşil; çekirdek testleri geçiyor; `git clone` → `make demo` ile lokalde çalışıyor.
5. Kullanıcı, bu dokümandaki her kararı bir cümleyle savunabiliyor (mülakat provası yapılacak).
