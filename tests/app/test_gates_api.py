...
def test_gate2_records_provider_usage_and_verifier_stage(tiny_corpus, monkeypatch):
    ...
    monkeypatch.setattr(
        "belge_gozu.answer.gemini.build_gemini_client",
        lambda *args, **kwargs: ProviderStub(),
    )
    data_dir, enc, _ = tiny_corpus
    settings = Settings(
        data_dir=data_dir,
